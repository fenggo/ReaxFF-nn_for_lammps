...
namespace ReaxFF {
  ...
  static void Compute_Bonded_Forces(reax_system *system,
                                    ...
                                    reax_list **lists)
  {
    BO(system, control,workspace, lists);
    Bonds(system,control, data, workspace, lists);
  ...
  static void Compute_Total_Force(reax_system *system, control_params *control, storage *workspace, reax_list **lists)
  {
    int i, pj;
    ...
    for (i = 0; i < system->N; ++i)
      for (pj = Start_Index(i, bonds); pj < End_Index(i, bonds); ++pj)
        if (i < bonds->select.bond_list[pj].nbr)
            Add_dBond_to_Forces(system, control,i, pj, workspace, lists);
  }
